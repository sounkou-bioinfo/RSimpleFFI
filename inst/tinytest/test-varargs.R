source("helper-testthat-compat.R")
...
test_that("ffi_cif_var with zero variadic args works", {
  ...
  cif <- ffi_cif_var(ffi_int(), nfixedargs = 2L, ffi_int(), ffi_int())
  expect_s3_class(cif, "RSimpleFFI::CIF")
})

test_that("ffi_cif_var with zero fixed args works", {
  # Edge case: all args are variadic
  cif <- ffi_cif_var(ffi_int(), nfixedargs = 0L, ffi_int(), ffi_int())
  expect_s3_class(cif, "RSimpleFFI::CIF")
})
...
test_that("varargs call with integers works", {
  ...
  # Sum 1 integer: 42
  cif1 <- ffi_cif_var(ffi_double(), nfixedargs = 1L, ffi_int(), ffi_int())
...
test_that("varargs call with doubles works", {
  ...
  # Single double
  cif1 <- ffi_cif_var(ffi_double(), nfixedargs = 1L, ffi_int(), ffi_double())

...
ffi_cif <- function(return_type, ...) {
  ...
  if (length(arg_types) > 0 && !all(sapply(arg_types, function(x) S7::S7_inherits(x, FFIType)))) {
...
ffi_symbol <- function(name, library = NULL) {
  ...
  if (is.null(library)) {
    ...
    if (is.null(info)) {
      stop("Symbol not found: ", name, ". Make sure the library containing this symbol is loaded.")
    }
...
S7::method(ffi_call, list(CIF, S7::class_character)) <- function(cif, symbol, ...) {

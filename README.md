...
**ABI Mode (Reflection-Based)**: Uses `ffi_struct()` to create struct
types where field offsets are computed at runtime using platform ABI
rules. Access fields with `ffi_get_field()` and `ffi_set_field()`

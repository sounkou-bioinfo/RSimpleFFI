test_that("bit-field struct passed by value treats as packed integer", {
  ...
  get_mode_fn <- ffi_function(
    "test_bitfield_struct_get_mode",
    ffi_int(), # return: int
    ffi_uint32()
  ) # arg: uint32_t (packed struct)
...
test_that("bit-field struct returned by value is packed integer", {
  ...
  create_fn <- ffi_function(
    "test_bitfield_struct_create",
...
test_that("bit-field struct by value round-trip works", {
  ...
  increment_fn <- ffi_function(
    "test_bitfield_struct_increment_priority",
...
test_that("PacketFlags 8-bit struct by value", {
  ...
  count_fn <- ffi_function("test_packet_count_flags", ffi_int(), ffi_uint8())

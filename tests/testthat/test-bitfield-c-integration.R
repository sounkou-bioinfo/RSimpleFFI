test_that("bit-field helpers work with actual C functions", {
  ...
  # Pack values in R
  packed <- settings$pack(list(enabled = 1L, mode = 5L, priority = 10L, reserved = 0L))
...
test_that("C-packed bit-fields can be unpacked in R", {
  ...
  # Pack in C
  pack_func <- ffi_function("test_bitfield_pack", ffi_uint32(), 
                           ffi_int(), ffi_int(), ffi_int())
...
test_that("bit-field modifications round-trip through C", {
  ...
  packed <- settings$pack(list(enabled = 1L, mode = 3L, priority = 5L, reserved = 0L))
  
  # Modify in C (increment priority)
  increment_func <- ffi_function("test_bitfield_increment_priority", 
  ...
  # Toggle in C
  toggle_func <- ffi_function("test_bitfield_toggle_enabled", 
                             ffi_uint32(), ffi_uint32())
...
test_that("C function can verify R-packed bit-fields", {
  ...
  # Pack in R
  packed <- settings$pack(list(enabled = 1L, mode = 7L, priority = 12L, reserved = 0L))
  
  # Verify in C
  verify_func <- ffi_function("test_bitfield_verify", ffi_int(), 
...
test_that("smaller bit-field structs work (8-bit)", {
  ...
  # Create SYN+ACK in R
  syn_ack <- packet$pack(list(syn = 1L, ack = 1L, fin = 0L, rst = 0L, reserved = 0L))
  ...
  fin_ack_rst <- packet$pack(list(syn = 0L, ack = 1L, fin = 1L, rst = 1L, reserved = 0L))
...
test_that("extreme bit-field values are handled correctly", {
  ...
  packed <- settings$pack(list(enabled = 1L, mode = 7L, priority = 15L, reserved = 0L))
  expect_equal(verify_func(as.integer(packed), 1L, 7L, 15L), 1L)
  
  # Test zero values
  packed <- settings$pack(list(enabled = 0L, mode = 0L, priority = 0L, reserved = 0L))
  ...
  packed <- settings$pack(list(enabled = 2L, mode = 8L, priority = 16L, reserved = 0L))

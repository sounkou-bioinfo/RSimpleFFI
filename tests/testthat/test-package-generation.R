...
test_that("generate_package_from_headers creates complete package structure", {
  ...
  expect_true(file.exists(file.path(tmpdir, "zzz.R")))
  
  # Check for generated binding files
  files <- list.files(tmpdir, pattern = "_bindings\\.R$")
  expect_true(length(files) > 0)
  
  # Check bindings file
  bindings <- readLines(file.path(tmpdir, files[1]))
  expect_true(any(grepl("r_add", bindings)))
  expect_true(any(grepl("Point", bindings)))
  
  # Check zzz file
  zzz <- readLines(file.path(tmpdir, "zzz.R"))

...
demo_htslib_features <- function() {
  ...
  # Decode features
  if (bitwAnd(features, 1) != 0) {
    message("  ✓ CRAM support")
  }
  if (bitwAnd(features, 2) != 0) {
    message("  ✓ GCS support")
  }
  if (bitwAnd(features, 4) != 0) {
    message("  ✓ S3 support")
  }
  if (bitwAnd(features, 256) != 0) {
    message("  ✓ libcurl support")
  }
...
demo_read_vcf_url <- function() {
  ...
  message(
    "This demonstrates RSimpleFFI + htslib with libcurl streaming genomics data!"
  )
}

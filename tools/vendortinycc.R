...
url <- paste0("https://github.com/TinyCC/tinycc/archive/", commit_hash, ".tar.gz")
...
if (mode == "download") {
  message("Downloading tinycc tarball (commit ", substr(commit_hash, 1, 7), ") to ", tarball_path)
...
if (mode == "unpack") {
  if (!file.exists(tarball_path)) {
    stop("tinycc tarball not found: ", tarball_path, "\nRun with 'download' mode first.")
  }

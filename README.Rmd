...
Current limitations include unnecessary copying, lack of protection, potential memory leaks, and type objects are C pointers that are never finalized. We do not support complex types, unions or special packing for alignment for Structs. This is ABI level access to C Routines in dynamic libraries, so care must be taken to alignment issues.

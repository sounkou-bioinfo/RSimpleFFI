...
You can load external shared libraries at runtime using RSimpleFFI's `dll_load()` and `dll_ffi_symbol()` functions. These are wrappers around R's native `dyn.load()` facilities.
...
Right now there are unimplemented features and limitations including uncessary copying, lack of protection and several potential memory leaks. The interface can and should be refined further. Furthermore we will vendor libffi in future releases to avoid dependency issues.

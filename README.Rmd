...
it requires libffi to be installed on windows along with pkg-config : this is always the case with recent [RTools](https://cran.r-project.org/bin/windows/Rtools/rtools45/news.html). On unix libffi is always built from source.
...
Right now there are unimplemented features and limitations including uncessary copying, lack of protection and several potential memory leaks. The interface can and should be refined further. Our types are C pointers never finzalized.

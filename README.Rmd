...
libffi does not support passing bit-field structs by value (they're compiler/platform-specific). The workaround is to model them as packed integers.
...
**You cannot use `ffi_struct()` for this.** libffi will fail or give incorrect results.

**The Solution:** Treat bit-field structs as their underlying integer type. Calculate size by summing bit widths and rounding up (1, 2, 4, or 8 bytes). For `SettingsFlags` above: 1+3+4+24 = 32 bits → use `ffi_uint32()`.
